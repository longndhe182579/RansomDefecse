...
static void RunStaticForPid(DWORD pid) {
    ...
    if (it != g_Collector.end()) ApplyStatic(it->second, r);
}
...
static void CallML(DWORD pid) {
    ...
    if (resp.verdict == Verdict::Offline) {
        /*
         * FAIL-SAFE. v3.0: _popen fail -> KillPID ngay. Quá gắt.
         * CoW đã cứu file rồi — không cần kill vội.
         */
        LOG_W("[ML] ML ENGINE OFFLINE. PID=%lu KHONG bi kill. "
            "Giu backup vo thoi han, de user tu quyet.", pid);
        return;
    }
...
static void MaintenanceThread() {
    ...
    while (g_Running) {
        ...
        if (tick % 5 == 0) {
            std::lock_guard<std::mutex> lk(g_Mtx);
            for (auto& [pid, pf] : g_Collector) pf.TickDecay();
        }
    }
}
